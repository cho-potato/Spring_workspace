...
<body class="hold-transition sidebar-mini layout-fixed">
					        ...
					        <div class="card">
					            ...
					            <div class="card-body table-responsive p-0">
					                <table class="table table-hover text-nowrap">
					                    ...
					                    <tbody>
					                    <!-- template은 디자인에 영향을 주지 않음,,, 자체로는 아무런 효과가 없고 그냥 뷰의 영역임을 지정한다 -->
	...
	<script type="text/javascript">
		...
		const product={
			...
			`,
			props:["obj", "num"], //props 외부에서 전달되는 데이터 받기용
			data(){ // 자바로 비유하면 인스턴스 변수 영역 정도?
				return{
					json:this.obj,
					n:this.num
				};
	...
	function pageLink(n) {
		...
		pager.init(app1.productList, n); // app1.productList에 이미 result가 있으므로
		...
		// 넘겨받은 페이지 변호를 이용하여 해당 페이지에 보여질 배열을 생성 후 
		// currentList에 대입(Vue의 변수인 currentList만 제어하면 디자인은 자동으로 바뀌니까)
		app1.currentList.splice(0, app1.currentList.length); // currentList가 비대해지는 것을 방지하기 위한 비우기
	...
	function getList() {
		$.ajax({
			...
			success:function(result, status, xhr) {
				// 서버에서 가져온 json 배열을 뷰의 템플릿이 바라보고 있는 productList에 대입만 하면 디자인은 알아서 변경된다\
				// 즉, 개발자는 데이터 제어에만 집중하면 된다
				...
				for(let i=pager.firstPage; i<=pager.lastPage; i++) {
					if(i > pager.totalPage)break; // 내가 가진 페이지 수를 넘어서면 반복문 빠져 나오게

...
public class JdbcGalleryDAO implements GalleryDAO{
	...
	public List selectAll() {
		// 내부적으로 jdbc를 사용하지만 다 숨겨놓음
		// 즉, 개발자로 하여금 기존의 jdbc의 상투적 코드를 작성할 필요없게했을뿐이지 기존 jdbc 맞다
		
		// jdbcTemplate.query(sql, new RowMapper<넣을 DTO>);
		List galleryList = jdbcTemplate.query("select * from gallery order by gallery_idx desc", new RowMapper<Gallery>() {
			...
			public Gallery mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				// Gallery DTO가 보유한 phjotoList 채우기
				String sql = "select * from photo where gallery_idx=?";
				// jdbcTemplate.query(sql, 바인드변수, 매퍼);

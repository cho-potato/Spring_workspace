...
const imagebox={
	...
	data(){
		return{
			/*
			props 용도 : 외부에서 전달될 속성값을 보관하기 위한 변수(각 템플릿마다 할당된 데이터 값)
	...
	methods:{
		delImg:function(idx){
			...
			//imageList안의 모든 json이 보유한 idx값을 비교하여,
			//일치하면 삭제 즉 배열에서 제거
...
function preview(files){
	...
	for(let i=0;i<files.length;i++){
		...
		//app1.imageList 배열에 존재하지 않을 경우에만
		if(checkDuplicate(file.name)<1){//1보다 작다 : 발견된 게 없다
			let reader=new FileReader();//파일 입력 스트림 생성
			reader.onload=(e)=>{
				//console.log("이미지 읽기 완료", e);
				//app1.src.push(e.target.result); src->imageList로 변경함
				
				console.log("file 정보는", file);
				console.log("src 정보는", e.target.result);
				
				//이미지 정보를 낱개로 관리하지 말고, 하나의 json으로 몰아서 마치 DTO처럼 관리해보자
				key++;
				let json=[];//empty
				
				//고유값 넣기, 추후 이미지 삭제시 고유값을 사용하려고
				json['key']=key;
				//파일명 넣기, 중복 이미지 체크시 사용
				json['name']=file.name;
				//img.src에 대입할 예정
				...
				console.log("key값은", key);
				
				console.log("i값은", i);
				app1.imageList.push(json);//모든 정보가 다 들어있음
...
function regist(){
	...
	*/
	...
	//파일 파라미터 채우기(2개 이상임)
	for(let i=0;i<app1.imageList.length;i++){
		let file =app1.imageList[i].file;//전송할 파일 끄집어 내기
		formData.append("photo", file);//파라미터가 배열화
	}
	...
	$.ajax({
		url:"/gallery/regist",
		type:"post",
		processData:false,//title=text&writer=ddd 문자열화 방지

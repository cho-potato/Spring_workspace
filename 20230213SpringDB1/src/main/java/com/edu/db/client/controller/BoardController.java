...
// 컴포넌트 스캔의 대상이 될 때는 어노테이션(@)을 이용한다
@Controller
public class BoardController {
	...
	// 글쓰기 요청 처리 1)
	// @RequestMapping(value="지어낼경로")
	@RequestMapping(value="board/regist", method=RequestMethod.POST)
	public ModelAndView regist (Board board) {
		logger.info("글쓰기 요청받음"); // 어느 패키지에 어느 경로인지 파악 가능
		// System.out.println("글쓰기 요청 받음2"); // 파악 X
		boardService.insert(board); // BoardException이 발생할 수 있음
		ModelAndView mav = new ModelAndView("redirect:/board/list");
		return mav;
	}
	// 목록 요청처리
	@RequestMapping(value="/board/list", method=RequestMethod.GET)
	// public ModelAndView getList() {}  AndView는 String으로 커버, Model은 매개변수에
